...
1. 打开配置文件解压得到【配置文件】和【花园明朝字体】，因为 macOS 不含部分生僻字，所以将花园明朝字体安装到字体册。
2. 点选菜单栏【ㄓ】里的**用户设定**（路径 `~/Library/Rime`）打开文件夹，将【配置文件】 里所有文件复制粘贴进去，并选择覆盖。
3. 点击【重新部署】（快捷键 `Control+Option+｀`）完成定制。

![](https://ww1.sinaimg.cn/large/008eZBHKgy1gn45fjrwu3j31e00c87e7.jpg)
> 注 1：修改任何文件都要“重新部署”方可生效；  
> 注 2：本配置里搜狗词库较大，首次部署时间会稍长。
